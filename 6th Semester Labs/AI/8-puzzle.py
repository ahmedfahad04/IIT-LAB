...
# ? Explored list contain only the STATES
# ? Frontier contain the NODES

# Frontier is where we store the nodes that
# we have seen but have not yet expand.
...
class StackFrontier:
    ...
    # it checks if the state is in the frontier
    ...
    def pop(self):
        if self.empty():
            ...
            self.frontier = self.frontier[:-1]  # From `0` to `end-1`
            return node
...
class QueueFrontier(StackFrontier):
    def pop(self):
        if self.empty():
            ...
            self.frontier = self.frontier[1:]   # From `1` to `end`
            return node
...
class Puzzle:
    ...
    # it checks if the state is in the explored list
...
start = np.array([[1,8,2], [0,4,3], [7,6,5]])
