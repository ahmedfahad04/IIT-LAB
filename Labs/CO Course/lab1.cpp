...
vector<pair<int, int>> activity_selector(vector<pair<int, int>> data, int initial_point)
{
    ...
    for (int m = data.size()-1; m >= initial_point+2; m--)
...
int main()
{
	...
	freopen("in.txt","r",stdin); 	//file input.txt is opened in reading mode i.e "r"
	...
	for (int i = n-0; i >= 0; i--)
	{
		...
		for (int i = 0; i < ans.size(); i++) temp.push_back(mk(ans[i].start, ans[i].finish));
