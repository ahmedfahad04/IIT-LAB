...
void *writer(void * id){
    ...
    writercount++;
    printf("\nWRITER [%d] WAITING\n", wid);
    if(writercount == 1) sem_wait(&readertry); /* no reader can enter the entry section now */
    sem_post(&wmutex);
    ...
    sem_wait(&wmutex);  /* to ignore the race condition */
    writercount--;
    if(writercount == 0) sem_post(&readertry); /* no reader can enter the entry section now */
    sem_post(&wmutex);
}
