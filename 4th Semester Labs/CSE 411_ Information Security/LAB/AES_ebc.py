...
d_table = [     # table for AES_ECB_DECRYPT
    [
        0xe, 0xb, 0xd, 0x9,
        0x9, 0xe, 0xb, 0xd,
        0xd, 0x9, 0xe, 0xb,
...
def convert_hex_key(key_value):

    index = 0
    for row in range(4):
        for col in range(4):
            key[col*4+row] = ord(key_value[index])
            index += 1
...
def text_to_matrix_conversion(data):
    ...
    for i in range(1, len(hex_matrix)):
        for col in range(4):
            for row in range(4):
                ...
                if  hex_matrix[i][row][col] != "0x2a":  #ignore '*' character
                    value = hex_matrix[i][row][col]
...
def AES_ECB_ENCRYPT(text, en_key):

    # list/array initialization
    st_matrix = []
    initial_matrix = text
    
    convert_hex_key(en_key)
    
    round_key = key_expansion()

    # convert text to hex
    # initial_matrix = text_to_matrix_conversion(text)
...
def AES_ECB_DECRYPT(cipherText):
    ...
    decrypted_text = hex_to_text(st_matrix)
...
if __name__ == '__main__':
    ...
    plaintext = text_to_matrix_conversion(plaintext)
    cipherText = AES_ECB_ENCRYPT(plaintext, encryption_key)
    decryptText = AES_ECB_DECRYPT(cipherText)
    
    print("\nCIPHER TEXT (EBC):",cipher_in_hex(cipherText))
    print("\nPLAIN TEXT:",hex_to_text(decryptText))
